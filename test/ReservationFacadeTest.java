...
public class ReservationFacadeTest {
    ...
    private HeadLabCoordinator instance;


    @Before
    public void setUp() throws Exception {
        // Reset HeadLabCoordinator singleton so each test starts fresh
        resetSingleton(HeadLabCoordinator.class, "instance");

        // Reset CSVDatabaseManager singleton to avoid stale data from other test classes
        resetSingleton(CSVDatabaseManager.class, "instance");

        // Clear any session state left by other test classes
    ...
    public void tearDown() throws Exception {
        // Clean up session so this class doesn't pollute others
        AppBackend.getInstance().getSession().setCurrentUser(null);

        // Reset singletons after each test for the same reason
        resetSingleton(HeadLabCoordinator.class, "instance");
        resetSingleton(CSVDatabaseManager.class, "instance");
    }
    ...
    public void createLabManager_1() {
        // No current user set — should fail
