...
public class PaymentPage extends JFrame {
	...
	public PaymentPage(User currentUser) {
		...
		// Build a readable payments table
		JTable paymentsTable = new JTable();
		paymentsTable.setFont(new Font("Trebuchet MS", Font.PLAIN, 16));
		paymentsTable.setRowHeight(28);
		DefaultTableModel tableModel = new DefaultTableModel(
			new String[]{"Payment ID", "Method", "Amount ($)"}, 0) {
			public boolean isCellEditable(int r, int c) { return false; }
		};

		List<Payment> payments = currentUser.getPayments();
		if (payments == null || payments.isEmpty()) {
			tableModel.addRow(new Object[]{"—", "No payments yet", "—"});
		} else {
			for (Payment p : payments) {
				tableModel.addRow(new Object[]{
					p.getPaymentId() != null ? p.getPaymentId() : "—",
					p.getMethod() != null ? p.getMethod().toString() : "—",
		...
		btnPayment.addActionListener(e -> { /* already here */ });
